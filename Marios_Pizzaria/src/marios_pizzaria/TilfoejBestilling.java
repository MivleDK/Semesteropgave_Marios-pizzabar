...
public class TilfoejBestilling {
    ...
    *Output:            Ingen. Går tilbage til menu
    ...
    public void tilfoejBestilling() throws IOException {
        ...
        System.out.print("Indtast telefonnummer tilknyttet ordren (Format: 1234567890): ");
    ...
    protected void fjernBestilling() {
        ...
        System.out.println("tast telefonnummeret på ordren der skal fjernes");
        String telnr = sc.next();
        int telnrInt = Integer.parseInt(telnr);

        for (int i = 0; i < alleOrdrerListe.size(); i++) {

            Ordre s = alleOrdrerListe.get(i);
            String ordreDerSkalFjernes = s.getTelefonnummer();
            int ordreDerSkalFjernesInt = Integer.parseInt(ordreDerSkalFjernes);

            if (telnrInt == ordreDerSkalFjernesInt) {
                alleOrdrerListe.remove(i);
                System.out.println("Ordren er fjernet");
            } else {
               // System.out.println("Ordren kunne ikke findes i systemet");
            }
